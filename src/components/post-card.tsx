...
export function PostCard({ post }: PostCardProps) {
  ...
  const user = community?.members.find((m) => m.userId === post.userId)?.user
          ...
          <div className="flex items-center justify-between">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {post.userId === currentUser.id && <DropdownMenuItem>Delete</DropdownMenuItem>}
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
      ...
      <CardContent className="pb-2 pt-1">
        ...
        {post.image && (
          <div className="mt-3 overflow-hidden rounded-md">
            <img src={post.image || "/placeholder.svg"} alt="Post attachment" className="h-auto w-full object-cover" />
          </div>
        )}
      </CardContent>
            ...
            {post.comments.length > 0 && (
              <div className="space-y-2">
                {post.comments.map((comment) => {
                  const commentUser = community?.members.find((m) => m.userId === comment.userId)?.user
                  return (
                    <div key={comment.id} className="flex gap-2">
                      <Avatar className="h-6 w-6">
                        <AvatarImage src={commentUser?.image || undefined} />
                        <AvatarFallback>{commentUser?.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <div className="flex-1 rounded-md bg-muted p-2">
                        <div className="flex items-center justify-between">
                          <p className="text-xs font-medium">{commentUser?.name}</p>
