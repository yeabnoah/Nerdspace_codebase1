                ...
                <div className="flex flex-1 items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    ...
                    <h1 className="text-[10px] text-muted-foreground sm:text-xs">
                      Nerd@<span className="text-purple-500">{post.user.nerdAt}</span>
                    </h1>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-2 flex w-full flex-1 flex-col items-start justify-center">
                {post.media && post.media.length > 0 && (
                  <div className={`${!post.shared && "mt-2 sm:mt-4"} grid w-[100%] flex-1 gap-1 sm:gap-2 ${getGridClass(post.media.length)}`}>
                    {post.media.length === 1 && (
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          setSelectedPostImages(post.media.map((m: { url: string }) => m.url));
                    ...
                    {post.media.length === 2 &&
                      post.media.map((media: { url: string }, mediaIndex: number) => (
                        <div
                          key={mediaIndex}
                    ...
                    {post.media.length >= 3 && (
                      <div className="grid h-[30vh] w-full max-w-[82vw] grid-cols-[auto_100px] gap-1 sm:h-[36vh] sm:grid-cols-[auto_120px] sm:gap-2 md:w-[36vw]">
                        ...
                        <div className="flex w-full flex-col gap-1 sm:gap-2">
                          {post.media.slice(1, 4).map((media: { url: string }, mediaIndex: number) => (
                            <div
                              key={mediaIndex}
                              className="relative h-full w-full"
                              onClick={(e) => {
